...
/// Game mode - time based or attempts based
enum GameMode {
  time,
  attempts;

  String get label => switch (this) {
    GameMode.time => 'Por Tiempo',
    GameMode.attempts => 'Por Intentos',
  };

  IconData get icon => switch (this) {
    GameMode.time => Icons.timer,
    GameMode.attempts => Icons.numbers,
  };
}
...
class GameConfig {
  final GameMode mode;
  final int timeInSeconds; // For time mode
  final int maxAttempts; // For attempts mode

  const GameConfig({
    required this.mode,
    this.timeInSeconds = 300, // 5 minutes default
    this.maxAttempts = 10, // 10 attempts default
  });

  static const timeMode5Min = GameConfig(mode: GameMode.time, timeInSeconds: 300);
  static const timeMode3Min = GameConfig(mode: GameMode.time, timeInSeconds: 180);
  static const timeMode1Min = GameConfig(mode: GameMode.time, timeInSeconds: 60);
  
  static const attempts10 = GameConfig(mode: GameMode.attempts, maxAttempts: 10);
  static const attempts5 = GameConfig(mode: GameMode.attempts, maxAttempts: 5);
  static const attempts3 = GameConfig(mode: GameMode.attempts, maxAttempts: 3);
}

/// Represents a word guess attempt
class WordGuess {
